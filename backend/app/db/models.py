...
class InviteTokenORM(Base):
    """Pre-validated signup invite tokens generated by admins."""
    __tablename__ = "invite_tokens"

    id = Column(Text, primary_key=True, default=lambda: f"inv_{uuid.uuid4().hex[:12]}")
    token_hash = Column(Text, nullable=False)            # SHA-256 hash of the raw token
    created_by = Column(Text, nullable=False)             # admin user_id who created it
    role = Column(Text, nullable=False, default="user")   # role assigned on signup
    email = Column(Text, nullable=True)                   # optional: restrict to specific email
    label = Column(Text, nullable=True)                   # admin-visible label (e.g. "For John")
    max_uses = Column(Integer, nullable=False, default=1) # 1 = single-use
    ...
    def __repr__(self) -> str:
        return f"<InviteToken id={self.id!r} revoked={self.revoked}>"
